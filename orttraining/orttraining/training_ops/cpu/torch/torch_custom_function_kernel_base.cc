...
void PythonOpBase::Init(const OpKernelInfo& info) {
  ...
  fw_alias_input_ = info.GetAttrsOrDefault("tensor_reuse_map", std::vector<int64_t>((info.node().OutputDefs().size()), -1));
...
void PythonOpBase::SetOtherOutputs(OpKernelContext* context, std::vector<OrtValue>& returned_ortvalues) const {
  auto* ctx_internal = reinterpret_cast<onnxruntime::OpKernelContextInternal*>(context);
  ORT_ENFORCE(returned_ortvalues.size() == fw_alias_input_.size() - 1, "PythonOp output count mismatch inplace map count.",
              returned_ortvalues.size(), " != ", fw_alias_input_.size() - 1);
  for (size_t i = 0; i < returned_ortvalues.size(); ++i) {
    size_t output_index = i + 1;
    if (fw_alias_input_[output_index] != -1) {
      const void* tensor_address = returned_ortvalues[i].Get<Tensor>().DataRaw();
      const void* input_tensor_address = context->Input<Tensor>(fw_alias_input_[output_index])->DataRaw();
