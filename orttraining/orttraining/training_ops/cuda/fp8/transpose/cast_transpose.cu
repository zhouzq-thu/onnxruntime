  ...
  do {                                                                                                               \
    ...
    kernel<nvec_in, nvec_out, fp32, InputType, OutputType>                                                           \
        <<<n_blocks, cast_transpose_num_threads,                                                                     \
           cast_transpose_num_threads / n_warps_per_tile*(THREADS_PER_WARP + 1) * sizeof(Vec<OutputType, nvec_out>), \
           stream>>>(input_data, cast_output_data, transposed_output_data, scale, amax, row_length, num_rows,        \
                     n_tiles);                                                                                       \
  } while (false)
  ...
  do {                                                                                                              \
    ...
    const bool full_tile =                                                                                          \
        row_length % (nvec_in * THREADS_PER_WARP) == 0 && num_rows % (nvec_out * THREADS_PER_WARP) == 0;            \
                                                                                                                    \
    if (full_tile) {                                                                                                \
      LAUNCH_KERNEL(cast_transpose_kernel, nvec_in, nvec_out, n_tiles, n_blocks, InputType, OutputType);            \
    } else {                                                                                                        \
      LAUNCH_KERNEL(cast_transpose_kernel_notaligned, nvec_in, nvec_out, n_tiles, n_blocks, InputType, OutputType); \
    }                                                                                                               \
  } while (false)
...
SPECIALIZED_CAST_TRANSPOSE_IMPL(fp16, fp8e4m3)
