...
void CastTransposeBias(cudaStream_t stream, const InputType* input_data, OutputType* cast_output_data,
  ...
  param.output_c = cast_output_data;
  param.output_t = transposed_output_data;
  ...
  reduce_dbias<InputType>(workspace, dbias_data, row_length, num_rows, nvec_out, stream);
}
...
SPECIALIZED_CAST_TRANSPOSE_BIAS_IMPL(fp16, fp8e5m2)
