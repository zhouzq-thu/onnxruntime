...
InlinedVector<std::unique_ptr<GraphTransformer>> GenerateTransformers(
  ...
  switch (level) {
    ...
    case TransformerLevel::Level3: {
      ...
      // NchwcTransformer should have a higher priority versus this. Because NchwcTransformer does similar things
      // of fusing patterns and targeting CPU. However, NchwcTransformer will reorder the layout to nchwc which is only
      // available on x86-64, and not edge architectures like ARM. This transformer could be used by opencl-ep/cpu-ep,
      // so we will prefer NhwcTransformer once ort runs on x86-64 CPU, otherwise ConvAddActivationFusion is enabled.
