...
class CustomFuncOpKernelInfo:
    ...
    def __init__(self, kernel_invoke_id: str):
        ...
        self.position_to_tensor_index_map: Optional[Dict[int, int]] = None
...
def _get_context(forward_tensor_outputs: List[torch.Tensor]) -> Tuple[any, Optional[torch.Tensor]]:
    ...
    def _cond(t):
        if not isinstance(t, torch.Tensor) or not hasattr(t, "grad_fn"):
            return False
    ...
    if ts:
        ...
        ctx = first_tensor_output.grad_fn
...
def _tensor_handle(input_position, arg, grad_flag, position_to_tensor_index_map, raw_input_tensors_used_inplace,
                   input_tensors_used_for_fw_run, is_training_mode, inplace_map, is_first_time_run,
                   tensor_input_indices_to_save_in_ctx, tensor_input_indices_for_mark_dirty):
    ...
    *args,
    ...
    try:
        ...
        if kernel_info.position_to_tensor_index_map is None:
            ...
            kernel_info.position_to_tensor_index_map = position_to_tensor_index_map
