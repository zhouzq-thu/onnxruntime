...
struct Reducer : public Reducer<T, 1, WARPS_M, WARPS_N> {
  ...
  inline __device__ Reducer(const Params& params, uint32_t bidm, uint32_t bidn, uint32_t warp_m, uint32_t warp_n,
...
struct DynamicReducer : public Reducer<T, 1, WARPS_M, WARPS_N> {
  ...
  template <typename Params>
  inline __device__ DynamicReducer(const Params& params, uint32_t bidm, uint32_t bidn, uint32_t warp_m, uint32_t warp_n,
                                   uint32_t lane, void* smem)
      : Base(params, bidm, bidn, warp_m, warp_n, lane, smem),
...
struct Stats {
  ...
  template <typename Params>
  inline __device__ Stats(const Params& params, uint32_t bidm, uint32_t bidn, uint32_t warp_m, uint32_t warp_n,
                          uint32_t lane, void* smem)
      : inter_cta_(params.barrier, bidm, params.ctas_per_col, CTAS_PER_ROW),
  ...
  inline __device__ stats_t compute(const T (&elts)[N], const T rn) {
    ...
    if (lane_ < CTAS_PER_ROW) {
      ...
      m = onnxruntime::cuda::fp8::Get<0>::of<stats_t, T>(result);
      m2 = onnxruntime::cuda::fp8::Get<1>::of<stats_t, T>(result);
    }
