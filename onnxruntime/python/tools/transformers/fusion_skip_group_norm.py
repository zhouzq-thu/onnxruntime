...
class FusionSkipGroupNorm(Fusion):
    ...
    def get_skip_index(self, add, is_nhwc: bool):
        skip = -1
        broadcast = False
        if self.shape_infer_helper is None:
            ...
            if len(shape_a) == 4 and len(shape_b) == 4:
                if shape_a == shape_b:
                    ...
                    c = 3 if is_nhwc else 1
                    h = 1 if is_nhwc else 2
                    w = 2 if is_nhwc else 3
    ...
    def fuse(self, node, input_name_to_nodes, output_name_to_node):
        ...
        #     return False
        # if sum(self.fused_count.values()) > 1:
        #     return False
        ...
        skip, broadcast = self.get_skip_index(add, is_nhwc=False)
        if skip < 0:
            return False
